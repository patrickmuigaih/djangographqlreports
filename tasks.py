...
def coverage(c):
    ...
    """
    c.run("coverage run --source graphql_reports runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/graphql_reports.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ graphql_reports")
...
def lint(c):
    ...
    """
    c.run("flake8 graphql_reports tests")
...
def release(c, bumpsize=''):
    ...
    import graphql_reports
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=graphql_reports.__version__))
    c.run("git push --tags")
